...
for ((i=1; i<=10000; i++)); do
    if [ "$game" == "cave" ]; then
        ...
        --solver pysat-rc2 pysat-rc2-boolonly clingo-be scipy --reach-unreachable --out-tlvl-none --out-result-none
    fi
    if [ "$game" == "platform" ]; then
    ...
    if [ "$game" == "vertical" ]; then
        ...
        --size 20 16 --pattern-hard --count-soft --reach-start-goal b-t 8 --reach-move supercat-new \
        --solver pysat-rc2 pysat-rc2-boolonly clingo-be scipy --reach-unreachable --out-tlvl-none --out-result-none
    fi
    if [ "$game" == "sokoban" ]; then
        echo "no unplayable level for sokoban"
    fi
    if [ "$game" == "cavedoors" ]; then
        r=$((1 + $RANDOM % 3))
        if [ $r == 1 ]; then
            python3 ./sturgeon/scheme2output.py  --random $RANDOM --outfile $root_folder/$i --schemefile $root_folder/cave-doors.scheme \
            --size 16 16 --pattern-hard \
            --reach-junction { tl 4 \
            --reach-junction } br 4 \
            ...
            --reach-connect "--src { --dst b --move maze --unreachable" \
            ...
            --reach-connect "--src b --dst B --move maze --unreachable" \
            --reach-connect "--src B --dst } --move maze" \
            --reach-connect "--src { --dst } --move maze --unreachable" \
            --reach-connect "--src b --dst } --move maze --unreachable" \
            --solver pysat-rc2 pysat-rc2-boolonly clingo-be scipy --out-tlvl-none --out-result-none
    ...
    if [ "$game" == "caveportal" ]; then
        ...
        if [ $r == 1 ]; then
            ...
            --reach-connect "--src 0 --dst 1 --move maze --unreachable" --reach-connect "--src 2 --dst 3 --move maze" \
            ...
            --size 16 16 --pattern-hard --count-soft \
            --reach-junction 0 tl 4 --reach-junction 1 tr 4 --reach-junction 2 bl 4 --reach-junction 3 br 4 \
            --reach-connect "--src 0 --dst 1 --move maze" --reach-connect "--src 2 --dst 3 --move maze --unreachable" \
            --reach-connect "--src 0 --dst 2 --move maze --unreachable" \
            --solver pysat-rc2 pysat-rc2-boolonly clingo-be scipy --out-tlvl-none --out-result-none
        fi
    fi
